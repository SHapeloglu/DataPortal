...
def tablo_yetkili_mi(tablo_adi, kullanici_id):
    """Kullanıcının rolüne bu tablo atanmış mı? Admin her tabloya erişebilir."""
    try:
        # Admin her tabloya erişebilir
        if admin_mi(kullanici_id):
            return True
        yetkili = query("""
...
def api_excel_upload():
    ...
    try:
        ...
        logger.info(f"Sütun sayısı: {len(columns)}, ilk 3: {columns[:3]}")
        safe_columns = [sanitize(c) for c in columns]
        logger.info(f"Safe sütun sayısı: {len(safe_columns)}, ilk 3: {safe_columns[:3]}")
        cols_def = ', '.join([f"`{c}` LONGTEXT" for c in safe_columns])
        create_sql = f"CREATE TABLE `{table_name}` ({cols_def}) ROW_FORMAT=DYNAMIC"
        logger.info(f"CREATE SQL uzunluk: {len(create_sql)}, baş: {create_sql[:300]}")
        cur.execute(create_sql)
        logger.info("CREATE TABLE başarılı")
        _tablo_log_kaydet(cur, table_name, session.get('kullanici_id'), 'OLUSTUR', 0, 0, 0)
        # Oluşturan kullanıcının rolüne otomatik tablo yetkisi ver
        rol_id = session.get('rol_id')
        if rol_id:
            cur.execute("""
                INSERT IGNORE INTO rol_tablo (RolID, TablAdi) VALUES (%s, %s)
            """, (rol_id, table_name))
        conn.commit()
        conn.close()
        audit('TABLE_CREATE', f'Tablo: {table_name}')
        logger.info(f'Tablo oluşturuldu: {table_name} ({len(columns)} sütun)')
        sql = f"CREATE TABLE `{table_name}` (\n  " + ',\n  '.join([f"`{c}` LONGTEXT" for c in columns]) + "\n) ROW_FORMAT=DYNAMIC"
...
def api_excel_load():
    ...
    try:
        ...
        if 'ETL_DATE' not in db_cols:
            cur.execute(f"ALTER TABLE `{table_name}` ADD COLUMN `ETL_DATE` VARCHAR(30)")
            conn.commit()
            db_cols.append('ETL_DATE')

        # Sütun eşleştirme (tam, case-insensitive ve sanitize)
        db_cols_lower    = {c.lower(): c for c in db_cols}
        db_cols_sanitize = {sanitize(c).lower(): c for c in db_cols}
        matched_pairs = []
        for ec in df.columns:
            if ec in db_cols and ec != 'ETL_DATE':
                ...
                san = sanitize(ec).lower()
                if san in db_cols_sanitize and db_cols_sanitize[san] != 'ETL_DATE':
                    matched_pairs.append((ec, db_cols_sanitize[san]))
...
def dosya_db_aktar(dosya_id):
    ...
    try:
        ...
        if 'ETL_DATE' not in db_cols:
            c = conn.cursor()
            c.execute(f"ALTER TABLE `{tablo_adi}` ADD COLUMN `ETL_DATE` VARCHAR(30)")
            conn.commit()
            c.close()
            db_cols.append('ETL_DATE')
