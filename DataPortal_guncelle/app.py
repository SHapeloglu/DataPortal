...
def api_dosya_yukle():
    try:
        ...
        return jsonify({
            ...
            'msg': 'Dosya yüklendi, onay bekleniyor.' if onay_gerekli else 'Dosya yüklendi.'
        })
    except Exception as e:
...
def scheduler_yukle():
    ...
    try:
        ...
        gorevler = query("SELECT * FROM cron_gorev WHERE Aktif=1")
        for g in gorevler:
            try:
                scheduler.add_job(
                    cron_sp_calistir,
...
# Scheduler'ı uygulama başlarken başlat
try:
    from apscheduler.schedulers.background import BackgroundScheduler
    scheduler.start()
    threading.Timer(3.0, scheduler_yukle).start()  # 3sn bekle DB hazır olsun
